...
Automatic update checks are disabled by default. Enable automatic patch updates with the `autoupdate` option.
...
To check for new versions without installing them, set `autoupdate` to `"notify"`.
